...
|---|---|---|---|
| L1 | GNet PHY / GLCP / GBA / GCT | signaling, clocks, phits, flit-to-PHY mapping, pair/channel use, capability/rate negotiation, local transmission grants | global addressing, sessions, names |
| L2 | DLP | 32-bit flits, hop-local VC association, bounded hop transfer, link integrity, receiver-credit accounting, protocol adaptation | global addresses, end-to-end sessions, directory lookup |
...
The dedicated GNet copper control channel is GLCP, not GDP/GCTL. Baseline VC2 is hop-local metadata associated with each 32-bit flit; it is not part of the flit data field. A PHY may divide a flit into multiple phits.

Link `CREDIT` is capacity for one complete flit, independent of phit width; `GRANT` is infrastructure scheduling permission.
