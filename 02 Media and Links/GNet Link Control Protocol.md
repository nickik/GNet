...
Every advanced NIC begins in the Minimum GNet-3 compatibility mechanism. GNet-10, future GNet-20, larger buffers, future wider VC identifiers such as VC4/VC8, and other features are enabled only after both sides agree.
...
| `CREDIT` | receiver advertises guaranteed free capacity in complete GNet flits |
...
> **1 CREDIT = guaranteed downstream receive capacity for exactly one complete 32-bit data flit and its associated link metadata.**

Credit accounting is independent of the PHY phit width or the number of phits used to transfer one flit. A receiver may return credit in batches such as `+4`, `+8`, or `+16`; accounting remains exact to one flit.
...
The current engineering target is approximately **1 Mbit/s logical control signaling per direction** on the dedicated control pairs. A compact 16-bit control word would occupy about 16 microseconds before line-code overhead, which is short enough to pipeline control during an 8-flit GNet-10 scheduling quantum (25.6 microseconds at 10 Mbit/s of flit data).

The GNet-10 named rate is the 32-bit flit-data rate. A PHY that serializes the 2-bit VCID inline therefore needs 10.625 Mbit/s of link-bit capacity before any additional line coding to sustain 10 Mbit/s of flit data.
...
GNet-20 moves these semantics in-band after a negotiated mode transition; its reserved control-symbol/phit encoding remains open.
